...
print("\n" + "="*70)
print("TESTING SPEED EXTRACTION:")
test_result = extract_speed_mode("slowly move right")
print(f"  'slowly move right' -> {test_result}")
if test_result[0] == "slow":
    print("  ✓ WORKING!")
else:
    print(f"  ✗ BROKEN! Expected 'slow', got '{test_result[0]}'")
print("="*70 + "\n")
...
PHRASE_LIST = [
    ...
    # Speed modifiers for smooth motion control
    "slowly", "slow", "very slowly", "carefully", "careful", "gently", 
    "gentle", "quickly", "quick", "fast", "rapidly", "rapid", 
    "messily", "messy", "roughly", "aggressively"
]
...
def parse_move_command(text: str):
    """
    Parse movement commands from text with speed modifier support.
    
    Returns:
        tuple: (position_dict, speed_mode) where position_dict has x,y,z
               or (None, "default") if no valid command
    
    Examples:
        "move right slowly" -> ({"x": 0.1, "y": 0.567, "z": -0.24}, "slowly")
        "go forward 5 cm quickly" -> ({"x": 0.0, "y": 0.567, "z": -0.19}, "quickly")
    """
    # Extract speed mode FIRST before parsing movement
    speed_mode, clean_text = extract_speed_mode(text)
    
    # Now parse movement from cleaned text
    ...
    # Try sequence interpreter first if enabled
    if ENABLE_SEQUENCES and sequence_interpreter:
        try:
            actions = sequence_interpreter.interpret(text)
            if actions:
                learning_stats["sequence_interpretations"] += 1
                return None, "default"  # Sequence system handles this differently
        except Exception:
            pass
    ...
    direction = None
    axis_sign = None
    for dir_word, (axis, sign) in direction_map.items():
        if dir_word in text:
            direction = dir_word
            axis_sign = (axis, sign)
            break
    ...
    # Extract distance with qualifiers
    distance = 1.0  # default
    
    # Check for qualifiers
    if any(word in text for word in ["tiny", "teensy"]):
        distance = 0.3
    elif any(word in text for word in ["little bit", "slightly", "small"]):
        distance = 0.5
    elif any(word in text for word in ["large", "big"]):
        distance = 2.0
    
    # Check for explicit measurements
    number_match = re.search(r'(\d+(?:\.\d+)?)\s*(cm|centimeter|centimeters|mm|millimeter|millimeters)?', text)
    if number_match:
        value = float(number_match.group(1))
        unit = number_match.group(2) if number_match.group(2) else None
        
        if unit and unit.startswith('mm'):
            distance = value / 100.0  # Convert mm to units
        elif unit and unit.startswith('cm'):
            distance = value / 10.0   # Convert cm to units
        else:
            distance = value * DISTANCE_SCALE
...
def execute_learned_intent(result: dict, original_text: str):
    ...
    if intent == "move_relative":
        ...
        delta = parse_move_command(cmd_text)
...
def process_multi_command_sentence(sentence: str):
    """
    Process a sentence that may contain multiple commands.
    
    Returns:
        tuple: (positions_list, speed_mode)
               positions_list is a list of position dicts
               speed_mode is the detected speed for all commands
    
    Examples:
        "move right and up slowly" -> ([pos1, pos2], "slowly")
        "go forward then left" -> ([pos1, pos2], "default")
    """
    # Extract global speed mode for entire sentence
    speed_mode, clean_sentence = extract_speed_mode(sentence)
    
    # Split into individual commands
    commands = split_into_commands(clean_sentence)
    
    positions = []
    
    for cmd_text, combine in commands:
        # Parse each command (which might have its own speed)
        position, cmd_speed = parse_move_command(cmd_text)
        
        # Use command-specific speed if detected, otherwise use sentence-level speed
        if cmd_speed != "default":
            speed_mode = cmd_speed  # Override with more specific speed
        
        if position:
            positions.append(position)
    
    return positions, speed_mode


def add_positions_to_queue(positions, speed_mode="default"):
    """
    Add positions to the command queue with speed mode.
    
    Args:
        positions: List of position dicts with x, y, z keys
        speed_mode: Speed mode for the motion
                   Options: "slowly", "carefully", "quickly", "messily", "default"
    """
    ...
    with queue_lock:
        for pos in positions:
            # Add speed mode to each position
            pos_with_speed = apply_speed_to_command(pos.copy(), speed_mode)
            command_queue.append(pos_with_speed)
    
    # Write the last command to file immediately
    last_pos = apply_speed_to_command(positions[-1].copy(), speed_mode)
    
    try:
        with open(COMMAND_QUEUE_FILE, 'w') as f:
            json.dump(last_pos, f, indent=2)
        
        if speed_mode != "default":
            print(f"{get_timestamp()} -> Queue updated (speed: {speed_mode})")
    except Exception as e:
        print(f"{get_timestamp()} [ERROR] Failed to write command file: {e}")
...
class MicToAzureStream:
    ...
    def _execute_and_timeout(self, captured_text):
        ...
        with self.partial_lock:
            ...
            positions, speed_mode = process_multi_command_sentence(captured_text)
            if positions:
                add_positions_to_queue(positions, speed_mode)
                    ...
                    if final_text == executed_clean or final_text.startswith(executed_clean):
                        ...
                        if remaining and len(remaining) > 2:
                            if was_and_command:
                                ...
                                positions, speed_mode = process_multi_command_sentence(remaining)
                                if positions:
                                    add_positions_to_queue(positions, speed_mode)
                        ...
                        positions, speed_mode = process_multi_command_sentence(text)
                        if positions:
                            add_positions_to_queue(positions, speed_mode)
