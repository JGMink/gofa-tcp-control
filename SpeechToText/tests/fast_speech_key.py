...
USE_CLU = True  # Set to False to disable CLU calls for testing
...
last_processed_text = ""  # Track last processed command to avoid duplicates
...
class MicToAzureStream:
    ...
    def _on_recognizing(self, evt):
        """Handle partial recognition - check for emergency words and go trigger."""
        global last_processed_text
        text = evt.result.text
        print(f"[Partial] {text}", end='\r')  # Use carriage return for cleaner output
        ...
        # Check for "go" trigger word at the end to execute immediately
        text_lower = text.lower().strip().rstrip('.')
        if text_lower.endswith(GO_TRIGGER) or text_lower.endswith(GO_TRIGGER + "."):
            # Strip the "go" trigger and process the command
            command_text = re.sub(r'\s*\bgo\.?\s*$', '', text_lower, flags=re.IGNORECASE).strip()
            if command_text and not emergency_halt.is_set() and command_text != last_processed_text:
                print(f"\n[GO TRIGGER] Processing: '{command_text}'")
                positions = process_multi_command_sentence(command_text)
                if positions:
                    add_positions_to_queue(positions)
                    last_processed_text = command_text
    ...
    def _on_recognized(self, evt):
        ...
        if evt.result.reason == speechsdk.ResultReason.RecognizedSpeech:
            ...
            # Skip if already processed by go trigger
            text_for_compare = re.sub(r'\s*\bgo\.?\s*$', '', text.lower().strip().rstrip('.'), flags=re.IGNORECASE).strip()
            if text_for_compare == last_processed_text:
                print(f"[SKIP] Already processed via GO trigger")
                last_processed_text = ""  # Reset for next command
                return
